...
export const askDocument = async ({
  ...
  // 3. Restrict vector search to this user's document
  const filter = {
    must: [
      {
        key: "metadata.userId",
  ...
  // 4. Search only relevant chunks from this document
  ...
  // 5. Make sure we found something
  if (!results.length) {
    return {
      answer:
...
`;
  ...
  // 7. Ask Gemini using retrieved context
  const prompt = `
